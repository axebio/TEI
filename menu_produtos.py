...
class menu_produtos(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        titulo = tk.Label(self, text= "MENU PRODUTOS", font= "Helvetica 12 bold")
        titulo.pack(side= "top")


        consultar_button = tk.Button(self, text="CONSULTAR", width= 100, height= 3, command= lambda: controller.show_frame("prod_consulta"))
        consultar_button.pack(side = "top", padx= 20, pady= 5)

        cadastrar_button = tk.Button(self, text="CADASTRAR", width= 100, height= 3, command= lambda: controller.show_frame("prod_cadastro"))
        cadastrar_button.pack(side = "top", padx= 20, pady= 5)

        excluir_button = tk.Button(self, text="EXCLUIR", width= 100, height= 3, command= lambda: controller.show_frame("prod_delete"))
